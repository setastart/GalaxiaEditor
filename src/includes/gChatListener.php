...
foreach ($post['rooms'] ?? [] as $room => $roomData) {
    ...
    $roomsprefixed[Director::$mysqlConfig['db'] . ':rooms:' . $room] = $roomData['lastId'];

    if (!$redis->cmd('EXISTS', Director::$mysqlConfig['db'] . ':rooms:' . $room)->get())
        $redis->cmd('XADD', Director::$mysqlConfig['db'] . ':rooms:' . $room, '*', 'user', $me->id, 'create', t('Room Created'))->set();

    if ($room != '/edit/chat')
        $redis->cmd('EXPIRE', Director::$mysqlConfig['db'] . ':rooms:' . $room, TIMEOUT_ROOM_INACTIVE)->set();
...
while (!$messageActivity && !$userActivity && time() < $timeoutListen) {
    ...
    foreach ($post['rooms'] as $room => $roomData) {
        $clients = $redis->cmd('KEYS', Director::$mysqlConfig['db'] . ':editing:' . $room . ':*')->get();
...
$temp = $redis->cmd('HSCAN', Director::$mysqlConfig['db'] . ':userNames', '0')->get();
for ($i = 0; $i < count($temp[1]); $i += 2) {
    $r['users'][$temp[1][$i]]['name'] = $temp[1][$i + 1];
}

$temp = $redis->cmd('HSCAN', Director::$mysqlConfig['db'] . ':usersLastSeen', '0')->get();
for ($i = 0; $i < count($temp[1]); $i += 2) {
...
$prefixLength = strlen(Director::$mysqlConfig['db'] . ':rooms:');

<?php

return [


    // chat

    '%s is online' => [
        'en' => '%s is online',
        'pt' => '%s entrou',
        'es' => '%s ha entrado',
    ],
    '%s is offline' => [
        'en' => '%s is offline',
        'pt' => '%s saiu',
        'es' => '%s ha salido',
    ],
    '%s has disconnected' => [
        'en' => '%s has disconnected',
        'pt' => '%s desconectou',
        'es' => '%s se ha desconectado',
    ],
    'Send Message' => [
        'en' => 'Send Message',
        'pt' => 'Enviar Mensagem',
        'es' => 'Enviar Mensaje',
    ],
    'Message' => [
        'en' => 'Message',
        'pt' => 'Mensagem',
        'es' => 'Mensaje',
    ],
    'Messages' => [
        'en' => 'Messages',
        'pt' => 'Mensagems',
        'es' => 'Mensajes',
    ],








    // images

    'Image' => [
        'en' => 'Image',
        'pt' => 'Imagem',
        'es' => 'Imagen',
    ],
    'Images' => [
        'en' => 'Images',
        'pt' => 'Imagens',
        'es' => 'Imágenes',
    ],
    'Resize image to fit' => [
        'en' => 'Resize image to fit',
        'pt' => 'Redimensionar imagem',
        'es' => 'Redimensionar imágen',
    ],
    'Resize images to fit' => [
        'en' => 'Resize images to fit',
        'pt' => 'Redimensionar imagens',
        'es' => 'Redimensionar imágenes',
    ],
    'Resize' => [
        'en' => 'Resize',
        'pt' => 'Redimensionar',
        'es' => 'Redimensionar',
    ],
    'Resize Image' => [
        'en' => 'Resize Image',
        'pt' => 'Redimensionar imagem',
        'es' => 'Redimensionar imágen',
    ],
    'Delete Resizes' => [
        'en' => 'Delete Resizes',
        'pt' => 'Apagar Dimensões',
        'es' => 'Borrar Dimensiones',
    ],
    'Select' => [
        'en' => 'Select',
        'pt' => 'Selecionar',
        'es' => 'Selecionar',
    ],
    'Select Image' => [
        'en' => 'Select Image',
        'pt' => 'Selecionar Imagem',
        'es' => 'Selecionar Imagen',
    ],
    'Replace Image' => [
        'en' => 'Replace Image',
        'pt' => 'Substituir Imagem',
        'es' => 'Sustituir Imagen',
    ],
    'An image with the same slug already exists.' => [
        'en' => 'An image with the same slug already exists.',
        'pt' => 'Já existe uma imagem com o mesmo slug.',
        'es' => 'Ya existe una imágen con el mismo slug.',
    ],
    'Replace existing images' => [
        'en' => 'Replace existing images',
        'pt' => 'Substituir imagens existentes que tenham o mesmo slug',
        'es' => 'Sustituir imágenes existentes que tengan el mismo slug',
    ],
    'imgSlug' => [
        'en' => 'Image',
        'pt' => 'Imagem',
        'es' => 'Imagen',
    ],
    'Image Slug' => [
        'en' => 'Image Slug',
        'pt' => 'Slug da Imagem',
        'es' => 'Slug le la Imagen',
    ],
    'Image Slugs' => [
        'en' => 'Image Slugs',
        'pt' => 'Slugs de Imagens',
        'es' => 'Slugs de Imágenes',
    ],
    'Reorder Images' => [
        'en' => 'Reorder Images',
        'pt' => 'Reordenar Imagens',
        'es' => 'Reordenar Imágenes',
    ],
    'Delete Images' => [
        'en' => 'Delete Images',
        'pt' => 'Eliminar Imagens',
        'es' => 'Eliminar Imágenes',
    ],

    'Images to Delete' => [
        'en' => 'Images to Delete',
        'pt' => 'Imagens para Eliminar',
        'es' => 'Imágenes para eliminar',
    ],
    'Remove All' => [
        'en' => 'Remove All',
        'pt' => 'Remover Todas',
        'es' => 'Quitar Todas',
    ],
    'Remove None' => [
        'en' => 'Remove None',
        'pt' => 'Não Remover Nenhuma',
        'es' => 'No Quitar Ninguna',
    ],
    'Upload Images' => [
        'en' => 'Upload Images',
        'pt' => 'Carregar Imagens',
        'es' => 'Subir Imágenes',
    ],
    'In Use' => [
        'en' => 'In Use',
        'pt' => 'Em Uso',
        'es' => 'En Uso',
    ],
    'Filter In Use' => [
        'en' => 'Filter In Use',
        'pt' => 'Filtrar Em Uso',
        'es' => 'Filtrar En Uso',
    ],
    'Select All' => [
        'en' => 'Select All',
        'pt' => 'Seleccionar Todas',
        'es' => 'Selecionar Todas',
    ],
    'Deselect All' => [
        'en' => 'Deselect All',
        'pt' => 'Seleccionar Ninguna',
        'es' => 'Selecionar Ninguna',
    ],
    'Add Images' => [
        'en' => 'Add Images',
        'pt' => 'Adicionar Imagens',
        'es' => 'Añadir Imágenes',
    ],
    'Ignore' => [
        'en' => 'Ignore',
        'pt' => 'Ignorar',
        'es' => 'Ignorar',
    ],
    'Rename' => [
        'en' => 'Rename',
        'pt' => 'Renomear',
        'es' => 'Renombrar',
    ],








    // editor sections

    'Page' => [
        'en' => 'Page',
        'pt' => 'Página',
        'es' => 'Página',
    ],
    'Pages' => [
        'en' => 'Pages',
        'pt' => 'Páginas',
        'es' => 'Páginas',
    ],
    'Debug' => [
        'en' => 'Debug',
        'pt' => 'Debug',
        'es' => 'Debug',
    ],
    'History' => [
        'en' => 'History',
        'pt' => 'História',
        'es' => 'Historial',
    ],
    'History of %s' => [
        'en' => 'History of %s',
        'pt' => 'História de %s',
        'es' => 'Historial de %s',
    ],
    'Save %s in History' => [
        'en' => 'Save %s in History',
        'pt' => 'Guardar %s em História',
        'es' => 'Guardar %s en Historial',
    ],
    'Open History in a new tab' => [
        'en' => 'Open History in a new tab',
        'pt' => 'Abrir História numa nova aba',
        'es' => 'Abrir Historial en una pestaña nueva',
    ],
    'Tag' => [
        'en' => 'Tag',
        'pt' => 'Etiqueta',
        'es' => 'Etiqueta',
    ],
    'Tags' => [
        'en' => 'Tags',
        'pt' => 'Etiquetas',
        'es' => 'Etiquetas',
    ],
    'Statistics' => [
        'en' => 'Statistics',
        'pt' => 'Estatísticas',
        'es' => 'Estatísticas',
    ],
    'Blog' => [
        'en' => 'Blog',
        'pt' => 'Blog',
        'es' => 'Blog',
    ],
    'Blogs' => [
        'en' => 'Blogs',
        'pt' => 'Blogs',
        'es' => 'Blogs',
    ],
    'Post' => [
        'en' => 'Post',
        'pt' => 'Post',
        'es' => 'Post',
    ],
    'Posts' => [
        'en' => 'Posts',
        'pt' => 'Posts',
        'es' => 'Posts',
    ],
    'General' => [
        'en' => 'General',
        'pt' => 'Geral',
        'es' => 'General',
    ],
    'Global' => [
        'en' => 'Global',
        'pt' => 'Global',
        'es' => 'Global',
    ],
    'Globals' => [
        'en' => 'Globals',
        'pt' => 'Globais',
        'es' => 'Globales',
    ],
    'Global Field' => [
        'en' => 'Global Field',
        'pt' => 'Campo Global',
        'es' => 'Campo Global',
    ],
    'Global Fields' => [
        'en' => 'Global Fields',
        'pt' => 'Campos Globais',
        'es' => 'Campos Globales',
    ],
    'Other' => [
        'en' => 'Other',
        'pt' => 'Outro',
        'es' => 'Otro',
    ],
    'Others' => [
        'en' => 'Others',
        'pt' => 'Outros',
        'es' => 'Otros',
    ],
    'Service' => [
        'en' => 'Service',
        'pt' => 'Serviço',
        'es' => 'Servicio',
    ],
    'Services' => [
        'en' => 'Services',
        'pt' => 'Serviços',
        'es' => 'Servicios',
    ],
    'Project' => [
        'en' => 'Project',
        'pt' => 'Projecto',
        'es' => 'Proyecto',
    ],
    'Projects' => [
        'en' => 'Projects',
        'pt' => 'Projectos',
        'es' => 'Proyectos',
    ],
    'Programme' => [
        'en' => 'Programme',
        'pt' => 'Programa',
        'es' => 'Programa',
    ],
    'Programmes' => [
        'en' => 'Programmes',
        'pt' => 'Programas',
        'es' => 'Programas',
    ],
    'Team' => [
        'en' => 'Team',
        'pt' => 'Equipa',
        'es' => 'Equipa',
    ],
    'Teams' => [
        'en' => 'Teams',
        'pt' => 'Equipas',
        'es' => 'Equipas',
    ],
    'Page Field' => [
        'en' => 'Page Field',
        'pt' => 'Campo de Página',
        'es' => 'Campo de Página',
    ],
    'Page Fields' => [
        'en' => 'Page Fields',
        'pt' => 'Campos de Página',
        'es' => 'Campos de Página',
    ],
    'User' => [
        'en' => 'User',
        'pt' => 'Utilizador',
        'es' => 'Usuario',
    ],
    'Users' => [
        'en' => 'Users',
        'pt' => 'Utilizadores',
        'es' => 'Usuarios',
    ],








    // editor

    'Menu position' => [
        'en' => 'Menu position',
        'pt' => 'Posição do menu',
        'es' => 'Posición del menú',
    ],
    'Left' => [
        'en' => 'Left',
        'pt' => 'Esquerda',
        'es' => 'Izquierda',
    ],
    'Right' => [
        'en' => 'Right',
        'pt' => 'Direita',
        'es' => 'Derecha',
    ],
    'Editing' => [
        'en' => 'Editing',
        'pt' => 'A editar',
        'es' => 'A editar',
    ],
    'Replace' => [
        'en' => 'Replace',
        'pt' => 'Substituir',
        'es' => 'Sustituir',
    ],
    'Status' => [
        'en' => 'Status',
        'pt' => 'Estado',
        'es' => 'Estado',
    ],
    'Items' => [
        'en' => 'Items',
        'pt' => 'Ítens',
        'es' => 'Ítems',
    ],
    '+ Add new %s' => [
        'en' => '+ Add new %s',
        'pt' => '+ Adicionar %s',
        'es' => '+ Añadir %s',
    ],
    '+ Add' => [
        'en' => '+ Add',
        'pt' => '+ Adicionar',
        'es' => '+ Añadir',
    ],
    'Page Type' => [
        'en' => 'Page Type',
        'pt' => 'Tipo de Página',
        'es' => 'Tipo de Página',
    ],
    'Field' => [
        'en' => 'Field',
        'pt' => 'Campo',
        'es' => 'Campo',
    ],
    'Fields' => [
        'en' => 'Fields',
        'pt' => 'Campos',
        'es' => 'Campos',
    ],
    'Save' => [
        'en' => 'Save',
        'pt' => 'Guardar',
        'es' => 'Guardar',
    ],
    'Save and go back' => [
        'en' => 'Save and go back',
        'pt' => 'Guardar e voltar atrás',
        'es' => 'Guardar y volver atrás',
    ],
    'Save and add more' => [
        'en' => 'Save and add more',
        'pt' => 'Guardar e adicionar mais',
        'es' => 'Guardar y adicionar más',
    ],
    'Send' => [
        'en' => 'Send',
        'pt' => 'Enviar',
        'es' => 'Enviar',
    ],
    'Add and go back' => [
        'en' => 'Add and go back',
        'pt' => 'Adicionar e voltar atrás',
        'es' => 'Añadir y volver atrás',
    ],
    'Add these and more' => [
        'en' => 'Add these and more',
        'pt' => 'Adicionar estas e mais',
        'es' => 'Añadir estas y más',
    ],
    'Upload' => [
        'en' => 'Upload',
        'pt' => 'Carregar',
        'es' => 'Subir',
    ],
    'Upload these and more' => [
        'en' => 'Upload these and more',
        'pt' => 'Carregar estas e mais',
        'es' => 'Subir estas y más',
    ],
    'Upload and go back to %s' => [
        'en' => 'Upload and go back to %s',
        'pt' => 'Carregar e voltar a %s',
        'es' => 'Subir y volver a %s',
    ],
    'Save and go back to %s' => [
        'en' => 'Save and go back to %s',
        'pt' => 'Guardar e voltar a %s',
        'es' => 'Guardar y volver a %s',
    ],
    'Upload and go back to %s' => [
        'en' => 'Upload and go back to %s',
        'pt' => 'Carregar e voltar a %s',
        'es' => 'Subir y volver a %s',
    ],
    'Go back' => [
        'en' => 'Go back',
        'pt' => 'Voltar atrás',
        'es' => 'Volver atrás',
    ],
    'Add' => [
        'en' => 'Add',
        'pt' => 'Adicionar',
        'es' => 'Añadir',
    ],
    'Added' => [
        'en' => 'Added',
        'pt' => 'Adicionado',
        'es' => 'Añadido',
    ],
    'Close' => [
        'en' => 'Close',
        'pt' => 'Fechar',
        'es' => 'Cerrar',
    ],
    'Remove' => [
        'en' => 'Remove',
        'pt' => 'Remover',
        'es' => 'Remover',
    ],
    'Delete' => [
        'en' => 'Delete',
        'pt' => 'Apagar',
        'es' => 'Eliminar',
    ],
    'Update' => [
        'en' => 'Update',
        'pt' => 'Actualizar',
        'es' => 'Actualizar',
    ],
    'Updated' => [
        'en' => 'Updated',
        'pt' => 'Actualizado',
        'es' => 'Actualizado',
    ],
    'Delete %s' => [
        'en' => 'Delete %s',
        'pt' => 'Apagar %s',
        'es' => 'Eliminar %s',
    ],
    'Delete %s: %s?' => [
        'en' => 'Delete %s: %s?',
        'pt' => 'Apagar %s: %s?',
        'es' => 'Eliminar %s: %s?',
    ],
    'Added field: %s.' => [
        'en' => 'Added field: %s.',
        'pt' => 'Campo adicionado: %s.',
        'es' => 'Campo adicionado: %s.',
    ],
    'Updated field: %s.' => [
        'en' => 'Updated field: %s.',
        'pt' => 'Campo actualizado: %s.',
        'es' => 'Campo actualizado: %s.',
    ],
    'Deleted field: %s.' => [
        'en' => 'Deleted field: %s.',
        'pt' => 'Campo apagado: %s.',
        'es' => 'Campo borrado: %s.',
    ],
    'Empty fields deleted: %d' => [
        'en' => 'Empty fields deleted: %d',
        'pt' => 'Campos vazios apagados: %s.',
        'es' => 'Campos vacios borrados: %s.',
    ],
    'This action is permanent.' => [
        'en' => 'This action is permanent.',
        'pt' => 'Esta acção é permanente.',
        'es' => 'Esta acción es irreversible.',
    ],
    'Cancel' => [
        'en' => 'Cancel',
        'pt' => 'Cancelar',
        'es' => 'Cancelar',
    ],
    'Starts' => [
        'en' => 'Starts',
        'pt' => 'Começa',
        'es' => 'Empieza',
    ],
    'Ends' => [
        'en' => 'Location',
        'pt' => 'Termina',
        'es' => 'Termina',
    ],
    'Added: %s.' => [
        'en' => 'Added: %s.',
        'pt' => 'Adicionado: %s.',
        'es' => 'Adicionado: %s.',
    ],
    'Deleted: %s.' => [
        'en' => 'Deleted: %s.',
        'pt' => 'Apagado: %s.',
        'es' => 'Borrado: %s.',
    ],
    'Title' => [
        'en' => 'Title',
        'pt' => 'Título',
        'es' => 'Título',
    ],
    'Titles' => [
        'en' => 'Titles',
        'pt' => 'Títulos',
        'es' => 'Títulos',
    ],
    'Menu Title' => [
        'en' => 'Menu Title',
        'pt' => 'Título de Menu',
        'es' => 'Título de Menú',
    ],
    'Head Title' => [
        'en' => 'Head Title',
        'pt' => 'Título Head',
        'es' => 'Título Head',
    ],
    'Head Description' => [
        'en' => 'Head Description',
        'pt' => 'Descrição Head',
        'es' => 'Descripción Head',
    ],
    'SEO Title' => [
        'en' => 'SEO Title',
        'pt' => 'Título SEO',
        'es' => 'Título SEO',
    ],
    'SEO Titles' => [
        'en' => 'SEO Titles',
        'pt' => 'Títulos SEO',
        'es' => 'Títulos SEO',
    ],
    'SEO Description' => [
        'en' => 'SEO Description',
        'pt' => 'Descricão SEO',
        'es' => 'Descripción SEO',
    ],
    'SEO Descriptions' => [
        'en' => 'SEO Descriptions',
        'pt' => 'Descricões SEO',
        'es' => 'Descripciónes SEO',
    ],
    'Meta Description' => [
        'en' => 'Meta Description',
        'pt' => 'Descricão Meta',
        'es' => 'Descripción Meta',
    ],
    'Meta Descriptions' => [
        'en' => 'Meta Descriptions',
        'pt' => 'Descricões Meta',
        'es' => 'Descripciónes Meta',
    ],
    'Title and Subtitle' => [
        'en' => 'Title and Subtitle',
        'pt' => 'Título e Subtítulo',
        'es' => 'Título y Subtítulo',
    ],
    'Subtitle' => [
        'en' => 'Subtitle',
        'pt' => 'Subtítulo',
        'es' => 'Subtítulo',
    ],
    'Subtitles' => [
        'en' => 'Subtitles',
        'pt' => 'Subtítulos',
        'es' => 'Subtítulos',
    ],
    'Slug' => [
        'en' => 'Slug',
        'pt' => 'Slug',
        'es' => 'Slug',
    ],
    'Slugs' => [
        'en' => 'Slugs',
        'pt' => 'Slugs',
        'es' => 'Slugs',
    ],
    'Title and Slug' => [
        'en' => 'Title and Slug',
        'pt' => 'Título e Slug',
        'es' => 'Título y Slug',
    ],
    'Name' => [
        'en' => 'Name',
        'pt' => 'Nome',
        'es' => 'Nombre',
    ],
    'Names' => [
        'en' => 'Names',
        'pt' => 'Nomes',
        'es' => 'Nombres',
    ],
    'Type' => [
        'en' => 'Type',
        'pt' => 'Tipo',
        'es' => 'Tipo',
    ],
    'Types' => [
        'en' => 'Types',
        'pt' => 'Tipos',
        'es' => 'Tipos',
    ],
    'Key' => [
        'en' => 'Key',
        'pt' => 'Chave',
        'es' => 'Llave',
    ],
    'Keys' => [
        'en' => 'Keys',
        'pt' => 'Chaves',
        'es' => 'Llaves',
    ],
    'Unused' => [
        'en' => 'Unused',
        'pt' => 'Não usados',
        'es' => 'No usados',
    ],
    'Content' => [
        'en' => 'Content',
        'pt' => 'Conteúdo',
        'es' => 'Contenido',
    ],
    'Contents' => [
        'en' => 'Contents',
        'pt' => 'Conteúdos',
        'es' => 'Contenidos',
    ],
    'Description' => [
        'en' => 'Description',
        'pt' => 'Descrição',
        'es' => 'Descripción',
    ],
    'Descriptions' => [
        'en' => 'Descriptions',
        'pt' => 'Descrições',
        'es' => 'Descripciónes',
    ],
    'Default' => [
        'en' => 'Default',
        'pt' => 'Default',
        'es' => 'Default',
    ],
    'Draft' => [
        'en' => 'Draft',
        'pt' => 'Rascunho',
        'es' => 'Borrador',
    ],
    'Special' => [
        'en' => 'Special',
        'pt' => 'Especial',
        'es' => 'Especial',
    ],
    'Published' => [
        'en' => 'Published',
        'pt' => 'Publicado',
        'es' => 'Publicado',
    ],
    'Contact' => [
        'en' => 'Contact',
        'pt' => 'Contacto',
        'es' => 'Contacto',
    ],
    'Contacts' => [
        'en' => 'Contacts',
        'pt' => 'Contactos',
        'es' => 'Contactos',
    ],
    'Option' => [
        'en' => 'Option',
        'pt' => 'Opção',
        'es' => 'Opción',
    ],
    'Options' => [
        'en' => 'Options',
        'pt' => 'Opções',
        'es' => 'Opciones',
    ],
    'Password' => [
        'en' => 'Password',
        'pt' => 'Palavra Passe',
        'es' => 'Contraseña',
    ],
    'Passwords' => [
        'en' => 'Passwords',
        'pt' => 'Palavras Passe',
        'es' => 'Contraseñas',
    ],
    'Your Password' => [
        'en' => 'Your Password',
        'pt' => 'A sua Palavra Passe',
        'es' => 'Tu Contraseña',
    ],
    'New Password' => [
        'en' => 'New Password',
        'pt' => 'Nova Palavra Passe',
        'es' => 'Nueva Contraseña',
    ],
    'Repeat Password' => [
        'en' => 'Repeat Password',
        'pt' => 'Repetir Palavra Passe',
        'es' => 'Repetir Contraseña',
    ],
    'Permission' => [
        'en' => 'Permission',
        'pt' => 'Permissão',
        'es' => 'Permiso',
    ],
    'Permissions' => [
        'en' => 'Permissions',
        'pt' => 'Permissões',
        'es' => 'Permisos',
    ],
    'Prefix' => [
        'en' => 'Prefix',
        'pt' => 'Prefixo',
        'es' => 'Prefixo',
    ],
    'Import' => [
        'en' => 'Import',
        'pt' => 'Importar',
        'es' => 'Importar',
    ],
    'importer-jsonld' => [
        'en' => 'Import json-ld from url',
        'pt' => 'Importar json-ld desde url',
        'es' => 'Importar json-ld desde url',
    ],
    'importer-youtube' => [
        'en' => 'Import from Youtube url',
        'pt' => 'Importar desde url Youtube',
        'es' => 'Importar desde url Youtube',
    ],
    'importer-vimeo' => [
        'en' => 'Import from Vimeo url',
        'pt' => 'Importar desde url Vimeo',
        'es' => 'Importar desde url Vimeo',
    ],
    'action-3' => [
        'en' => 'Created',
        'pt' => 'Criado',
        'es' => 'Creado',
    ],
    'action-2' => [
        'en' => 'Modified',
        'pt' => 'Modificado',
        'es' => 'Modificado',
    ],
    'action-1' => [
        'en' => 'Saved',
        'pt' => 'Salvado',
        'es' => 'Salvado',
    ],
    'action-0' => [
        'en' => 'Deleted',
        'pt' => 'Eliminado',
        'es' => 'Eliminado',
    ],
    'Created' => [
        'en' => 'Created',
        'pt' => 'Criado',
        'es' => 'Creado',
    ],
    'Modified' => [
        'en' => 'Modified',
        'pt' => 'Modificado',
        'es' => 'Modificado',
    ],
    'Saved' => [
        'en' => 'Saved',
        'pt' => 'Salvado',
        'es' => 'Salvado',
    ],
    'Deleted' => [
        'en' => 'Deleted',
        'pt' => 'Eliminado',
        'es' => 'Eliminado',
    ],
    'Link' => [
        'en' => 'Link',
        'pt' => 'Link',
        'es' => 'Link',
    ],
    'Id' => [
        'en' => 'Id',
        'pt' => 'Id',
        'es' => 'Id',
    ],
    'Last Online' => [
        'en' => 'Last Online',
        'pt' => 'Última vez Online',
        'es' => 'Última vez Online',
    ],
    'Logout' => [
        'en' => 'Logout',
        'pt' => 'Terminar sessão',
        'es' => 'Cerrar sesión',
    ],
    'Reload' => [
        'en' => 'Reload',
        'pt' => 'Recarregar',
        'es' => 'Volver a cargar',
    ],
    'Visit' => [
        'en' => 'Visit',
        'pt' => 'Visitar',
        'es' => 'Visitar',
    ],
    'Email' => [
        'en' => 'Email',
        'pt' => 'Email',
        'es' => 'Email',
    ],
    'Emails' => [
        'en' => 'Emails',
        'pt' => 'Emails',
        'es' => 'Emails',
    ],
    '%s with id %s does not exist.' => [
        'en' => '%s with id %s does not exist.',
        'pt' => '%s com id %s não existe.',
        'es' => '%s con id %s no existe.',
    ],
    'Edit Item' => [
        'en' => 'Edit Item',
        'pt' => 'Editar Ítem',
        'es' => 'Editar Ítem',
    ],
    'Open in new tab' => [
        'en' => 'Open in new tab',
        'pt' => 'Abrir numa nova aba',
        'es' => 'Abrir en una nueva pestaña',
    ],
    'Preview' => [
        'en' => 'Preview',
        'pt' => 'Vista Prévia',
        'es' => 'Vista Previa',
    ],
    'Language' => [
        'en' => 'Language',
        'pt' => 'Idioma',
        'es' => 'Idioma',
    ],
    'Header' => [
        'en' => 'Header',
        'pt' => 'Cabeçalho',
        'es' => 'Encabezado',
    ],
    'Headers' => [
        'en' => 'Headers',
        'pt' => 'Cabeçalhos',
        'es' => 'Encabezados',
    ],
    'Footer' => [
        'en' => 'Footer',
        'pt' => 'Pé de página',
        'es' => 'Pié de página',
    ],
    'Footers' => [
        'en' => 'Footers',
        'pt' => 'Pés de página',
        'es' => 'Piés de página',
    ],








    // misc

    'Yes' => [
        'en' => 'Yes',
        'pt' => 'Sim',
        'es' => 'Si',
    ],
    'No' => [
        'en' => 'No',
        'pt' => 'Não',
        'es' => 'No',
    ],
    'Client' => [
        'en' => 'Client',
        'pt' => 'Cliente',
        'es' => 'Cliente',
    ],
    'Clients' => [
        'en' => 'Clients',
        'pt' => 'Clientes',
        'es' => 'Clientes',
    ],
    'Leave this field blank' => [
        'en' => 'Leave this field blank',
        'pt' => 'Não preencha este campo',
        'es' => 'No rellenar este campo',
    ],
    'initial' => [
        'en' => 'initial',
        'pt' => 'inicial',
        'es' => 'inicial',
    ],
    'undo' => [
        'en' => 'undo',
        'pt' => 'desfazer',
        'es' => 'deshacer',
    ],
    'Per page' => [
        'en' => 'Per page',
        'pt' => 'Por página',
        'es' => 'Por página',
    ],
    'Phone' => [
        'en' => 'Phone',
        'pt' => 'Telefone',
        'es' => 'Teléfono',
    ],
    'Phone Prefix' => [
        'en' => 'Phone Prefix',
        'pt' => 'Prefixo do Telefone',
        'es' => 'Prefixo del Teléfono',
    ],
    'Phones' => [
        'en' => 'Phones',
        'pt' => 'Telefones',
        'es' => 'Teléfonos',
    ],
    'Address' => [
        'en' => 'Address',
        'pt' => 'Morada',
        'es' => 'Dirección',
    ],
    'Addresses' => [
        'en' => 'Addresses',
        'pt' => 'Moradas',
        'es' => 'Direcciónes',
    ],
    'Postal code' => [
        'en' => 'Postal code',
        'pt' => 'Código postal',
        'es' => 'Código postal',
    ],
    'Note' => [
        'en' => 'Note',
        'pt' => 'Nota',
        'es' => 'Nota',
    ],
    'Notes' => [
        'en' => 'Notes',
        'pt' => 'Notas',
        'es' => 'Notas',
    ],
    'Organization' => [
        'en' => 'Organization',
        'pt' => 'Organização',
        'es' => 'Organización',
    ],
    'Caption' => [
        'en' => 'Caption',
        'pt' => 'Legenda',
        'es' => 'Leyenda',
    ],
    'Registration' => [
        'en' => 'Registration',
        'pt' => 'Inscrição',
        'es' => 'Inscripción',
    ],
    'Organizations' => [
        'en' => 'Organizations',
        'pt' => 'Organizações',
        'es' => 'Organizaciones',
    ],
    'Featured' => [
        'en' => 'Featured',
        'pt' => 'Destacado',
        'es' => 'Destacado',
    ],
    'Anniversary' => [
        'en' => 'Anniversary',
        'pt' => 'Aniversário',
        'es' => 'Aniversario',
    ],
    'Anniversaries' => [
        'en' => 'Anniversaries',
        'pt' => 'Aniversários',
        'es' => 'Aniversarios',
    ],
    'Quote' => [
        'en' => 'Quote',
        'pt' => 'Citação',
        'es' => 'Cita',
    ],
    'Quotes' => [
        'en' => 'Quotes',
        'pt' => 'Citações',
        'es' => 'Citas',
    ],
    'Author' => [
        'en' => 'Author',
        'pt' => 'Autora',
        'es' => 'Autora',
    ],
    'Authors' => [
        'en' => 'Authors',
        'pt' => 'Autoras',
        'es' => 'Autoras',
    ],
    'Synopsis' => [
        'en' => 'Synopsis',
        'pt' => 'Sinopse',
        'es' => 'Sinopse',
    ],
    'Event' => [
        'en' => 'Event',
        'pt' => 'Evento',
        'es' => 'Evento',
    ],
    'Events' => [
        'en' => 'Events',
        'pt' => 'Eventos',
        'es' => 'Eventos',
    ],
    'Location' => [
        'en' => 'Location',
        'pt' => 'Lugar',
        'es' => 'Lugar',
    ],
    'Activity' => [
        'en' => 'Activity',
        'pt' => 'Actividade',
        'es' => 'Actividad',
    ],
    'Activities' => [
        'en' => 'Activities',
        'pt' => 'Actividades',
        'es' => 'Actividades',
    ],
    'Icon' => [
        'en' => 'Icon',
        'pt' => 'Ícone',
        'es' => 'Icono',
    ],
    'Icons' => [
        'en' => 'Icons',
        'pt' => 'Ícones',
        'es' => 'Iconos',
    ],
    'Photo' => [
        'en' => 'Photo',
        'pt' => 'Foto',
        'es' => 'Foto',
    ],
    'Photos' => [
        'en' => 'Photos',
        'pt' => 'Fotos',
        'es' => 'Fotos',
    ],
    'Poster' => [
        'en' => 'Poster',
        'pt' => 'Cartaz',
        'es' => 'Cartel',
    ],
    'Posters' => [
        'en' => 'Posters',
        'pt' => 'Cartazes',
        'es' => 'Carteles',
    ],
    'Redirect' => [
        'en' => 'Redirect',
        'pt' => 'Redireccionamento',
        'es' => 'Redireccionamiento',
    ],
    'Redirects' => [
        'en' => 'Redirects',
        'pt' => 'Redireccionamentos',
        'es' => 'Redireccionamientos',
    ],
    'Free' => [
        'en' => 'Free',
        'pt' => 'Grátis',
        'es' => 'Gratis',
    ],
    'Thank you' => [
        'en' => 'Thank you',
        'pt' => 'Obrigada',
        'es' => 'Gracias',
    ],
    'Price' => [
        'en' => 'Price',
        'pt' => 'Preço',
        'es' => 'Precio',
    ],
    'Place' => [
        'en' => 'Place',
        'pt' => 'Lugar',
        'es' => 'Lugar',
    ],
    'Places' => [
        'en' => 'Places',
        'pt' => 'Lugares',
        'es' => 'Lugares',
    ],
    'Location' => [
        'en' => 'Location',
        'pt' => 'Local',
        'es' => 'Local',
    ],
    'Locations' => [
        'en' => 'Locations',
        'pt' => 'Locais',
        'es' => 'Locales',
    ],
    'Main menu' => [
        'en' => 'Main menu',
        'pt' => 'Menu principal',
        'es' => 'Menú principal',
    ],
    'This page in other languages' => [
        'en' => 'This page in other languages',
        'pt' => 'Esta página em outras linguas',
        'es' => 'Esta pagina em otras lenguas',
    ],
    'Accessibility icon' => [
        'en' => 'Accessibility icon',
        'pt' => 'Ícone da acessibilidade',
        'es' => 'Ícone de acessibilidade',
    ],
    'Mode' => [
        'en' => 'Mode',
        'pt' => 'Modo',
        'es' => 'Modo',
    ],
    'Screenshot' => [
        'en' => 'Screenshot',
        'pt' => 'Captura de ecrã',
        'es' => 'Captura de pantalla',
    ],








    // filters

    'Filter' => [
        'en' => 'Filter',
        'pt' => 'Filtro',
        'es' => 'Filtro',
    ],
    'Filters' => [
        'en' => 'Filter',
        'pt' => 'Filtros',
        'es' => 'Filtros',
    ],
    'Filter Status' => [
        'en' => 'Filter Status',
        'pt' => 'Filtrar Estados',
        'es' => 'Filtrar Estados',
    ],
    'Filter Keys and Contents' => [
        'en' => 'Filter Keys and Contents',
        'pt' => 'Filtrar Chaves e Conteúdos',
        'es' => 'Filtrar Llaves y Contenidos',
    ],
    'Filter Pages' => [
        'en' => 'Filter Pages',
        'pt' => 'Filtrar Páginas',
        'es' => 'Filtrar Páginas',
    ],
    'Filter Tables' => [
        'en' => 'Filter Tables',
        'pt' => 'Filtrar Tabelas',
        'es' => 'Filtrar Tabelas',
    ],
    'Filter Ids' => [
        'en' => 'Filter Ids',
        'pt' => 'Filtrar Ids',
        'es' => 'Filtrar Ids',
    ],
    'Filter Items' => [
        'en' => 'Filter Items',
        'pt' => 'Filtrar Ítens',
        'es' => 'Filtrar Ítems',
    ],
    'Filter Actions' => [
        'en' => 'Filter Actions',
        'pt' => 'Filtrar Ações',
        'es' => 'Filtrar Acciones',
    ],
    'Filter Keys' => [
        'en' => 'Filter Keys',
        'pt' => 'Filtrar Chaves',
        'es' => 'Filtrar Llaves',
    ],
    'Filter Contents' => [
        'en' => 'Filter Contents',
        'pt' => 'Filtrar Conteúdos',
        'es' => 'Filtrar Contenidos',
    ],
    'Filter Tags' => [
        'en' => 'Filter Tags',
        'pt' => 'Filtrar Etiquetas',
        'es' => 'Filtrar Etiquetas',
    ],
    'Filter Locations' => [
        'en' => 'Filter Locations',
        'pt' => 'Filtrar Locais',
        'es' => 'Filtrar Locales',
    ],
    'Filter Titles and Slugs' => [
        'en' => 'Filter Titles and Slugs',
        'pt' => 'Filtrar Títulos e Slugs',
        'es' => 'Filtrar Títulos y Slugs',
    ],
    'Filter Slugs' => [
        'en' => 'Filter Slugs',
        'pt' => 'Filtrar Slugs',
        'es' => 'Filtrar Slugs',
    ],
    'Filter Names and Emails' => [
        'en' => 'Filter Names and Emails',
        'pt' => 'Filtrar Nomes e Emails',
        'es' => 'Filtrar Nombres y Emails',
    ],
    'Filter Titles' => [
        'en' => 'Filter Titles',
        'pt' => 'Filtrar Títulos',
        'es' => 'Filtrar Títulos',
    ],
    'Filter Titles and Subtitles' => [
        'en' => 'Filter Titles and Subtitles',
        'pt' => 'Filtrar Títulos e Subtítulos',
        'es' => 'Filtrar Títulos y Subtítulos',
    ],
    'Filter Subtitles' => [
        'en' => 'Filter Subtitles',
        'pt' => 'Filtrar Subtítulos',
        'es' => 'Filtrar Subtítulos',
    ],
    'Filter Types' => [
        'en' => 'Filter Types',
        'pt' => 'Filtrar Tipos',
        'es' => 'Filtrar Tipos',
    ],
    'Filter Links' => [
        'en' => 'Filter Links',
        'pt' => 'Filtrar Links',
        'es' => 'Filtrar Links',
    ],
    'Filter Dates' => [
        'en' => 'Filter Dates',
        'pt' => 'Filtrar Datas',
        'es' => 'Filtrar Fechas',
    ],
    'Filter Programmes' => [
        'en' => 'Filter Programmes',
        'pt' => 'Filtrar Programas',
        'es' => 'Filtrar Programas',
    ],
    'Filter Alt' => [
        'en' => 'Filter Alt',
        'pt' => 'Filtrar Alt',
        'es' => 'Filtrar Alt',
    ],
    'Empty' => [
        'en' => 'Empty',
        'pt' => 'Vazio',
        'es' => 'Vacío',
    ],
    'Number of letters ❖ Number of words' => [
        'en' => 'Number of letters ❖ Number of words',
        'pt' => 'Número de letras ❖ Número de palavras',
        'es' => 'Número de letras ❖ Número de palabras',
    ],







    // errors, warnings, infos, devlogs

    'Error' => [
        'en' => 'Error',
        'pt' => 'Erro',
        'es' => 'Error',
    ],
    'Errors' => [
        'en' => 'Errors',
        'pt' => 'Erros',
        'es' => 'Errores',
    ],
    'Warning' => [
        'en' => 'Warning',
        'pt' => 'Aviso',
        'es' => 'Aviso',
    ],
    'Warnings' => [
        'en' => 'Warnings',
        'pt' => 'Avisos',
        'es' => 'Avisos',
    ],
    'Information' => [
        'en' => 'Information',
        'pt' => 'Informação',
        'es' => 'Información',
    ],
    'Informations' => [
        'en' => 'Informations',
        'pt' => 'Informações',
        'es' => 'Informaciones',
    ],
    'Devlog' => [
        'en' => 'Devlog',
        'pt' => 'Devlog',
        'es' => 'Devlog',
    ],
    'Devlogs' => [
        'en' => 'Devlogs',
        'pt' => 'Devlogs',
        'es' => 'Devlogs',
    ],
    'No changes were made.' => [
        'en' => 'No changes were made.',
        'pt' => 'Não houve alterações.',
        'es' => 'No ha habido modificaciones.',
    ],
    'Form errors found.' => [
        'en' => 'Form errors found.',
        'pt' => 'Foram encontrados erros.',
        'es' => 'Errores encontrados.',
    ],










    // dates

    'Date and Time' => [
        'en' => 'Date and Time',
        'pt' => 'Data e Hora',
        'es' => 'Fecha y Hora',
    ],
    'Date' => [
        'en' => 'Date',
        'pt' => 'Data',
        'es' => 'Fecha',
    ],
    'Dates' => [
        'en' => 'Dates',
        'pt' => 'Datas',
        'es' => 'Fechas',
    ],
    'Until' => [
        'en' => 'Until',
        'pt' => 'Até',
        'es' => 'Hasta',
    ],
    'Start Time' => [
        'en' => 'Start Time',
        'pt' => 'Hora de início',
        'es' => 'Hora de inicio',
    ],
    'End Time' => [
        'en' => 'End Time',
        'pt' => 'Hora de fim',
        'es' => 'Hora de fin',
    ],
    'Time' => [
        'en' => 'Time',
        'pt' => 'Hora',
        'es' => 'Hora',
    ],
    'Times' => [
        'en' => 'Times',
        'pt' => 'Horas',
        'es' => 'Horas',
    ],
    'Year' => [
        'en' => 'Year',
        'pt' => 'Ano',
        'es' => 'Año',
    ],
    'Month' => [
        'en' => 'Month',
        'pt' => 'Mês',
        'es' => 'Mes',
    ],
    'Day' => [
        'en' => 'Day',
        'pt' => 'Dia',
        'es' => 'Día',
    ],
    'January' => [
        'en' => 'January',
        'pt' => 'Janeiro',
        'es' => 'Enero',
    ],
    'February' => [
        'en' => 'February',
        'pt' => 'Fevereiro',
        'es' => 'Febrero',
    ],
    'March' => [
        'en' => 'March',
        'pt' => 'Março',
        'es' => 'Marzo',
    ],
    'April' => [
        'en' => 'April',
        'pt' => 'Abril',
        'es' => 'Abril',
    ],
    'May' => [
        'en' => 'May',
        'pt' => 'Maio',
        'es' => 'Mayo',
    ],
    'June' => [
        'en' => 'June',
        'pt' => 'Junho',
        'es' => 'Junio',
    ],
    'July' => [
        'en' => 'July',
        'pt' => 'Julho',
        'es' => 'Julio',
    ],
    'August' => [
        'en' => 'August',
        'pt' => 'Agosto',
        'es' => 'Agosto',
    ],
    'September' => [
        'en' => 'September',
        'pt' => 'Setembro',
        'es' => 'Septiembre',
    ],
    'October' => [
        'en' => 'October',
        'pt' => 'Outubro',
        'es' => 'Octobre',
    ],
    'November' => [
        'en' => 'November',
        'pt' => 'Novembro',
        'es' => 'Noviembre',
    ],
    'December' => [
        'en' => 'December',
        'pt' => 'Dezembro',
        'es' => 'Diciembre',
    ],

    'Duration' => [
        'en' => 'Duration',
        'pt' => 'Duração',
        'es' => 'Duration',
    ],

    'Day of Week' => [
        'en' => 'Day of Week',
        'pt' => 'Dia da Semana',
        'es' => 'Día de la Semana',
    ],

    'dow 0' => ['en' => 'Sun', 'pt' => 'Dom', 'es' => 'Dom'],
    'dow 1' => ['en' => 'Mon', 'pt' => 'Seg', 'es' => 'Lun'],
    'dow 2' => ['en' => 'Tue', 'pt' => 'Ter', 'es' => 'Mar'],
    'dow 3' => ['en' => 'Wed', 'pt' => 'Qua', 'es' => 'Mie'],
    'dow 4' => ['en' => 'Thu', 'pt' => 'Qui', 'es' => 'Jue'],
    'dow 5' => ['en' => 'Fri', 'pt' => 'Sex', 'es' => 'Vie'],
    'dow 6' => ['en' => 'Sat', 'pt' => 'Sáb', 'es' => 'Sab'],
    'dow 7' => ['en' => 'Sun', 'pt' => 'Dom', 'es' => 'Dom'],

    'Tomorrow' => [
        'en' => 'Tomorrow',
        'pt' => 'Amanhã',
        'es' => 'Mañana',
    ],
    'Soon' => [
        'en' => 'Soon',
        'pt' => 'Proximamente',
        'es' => 'Proximamente',
    ],
    'In progress' => [
        'en' => 'In progress',
        'pt' => 'A decorrer',
        'es' => 'A decorrer',
    ],

    'Registration' => [
        'en' => 'Registration',
        'pt' => 'Inscrição',
        'es' => 'Inscripción',
    ],
    'Registrations' => [
        'en' => 'Registrations',
        'pt' => 'Inscriçãos',
        'es' => 'Inscripciones',
    ],

    'Online' => [
        'en' => 'Online',
        'pt' => 'Online',
        'es' => 'Online',
    ],
    'Offline' => [
        'en' => 'Offline',
        'pt' => 'Offline',
        'es' => 'Offline',
    ],
    'On/Offline' => [
        'en' => 'On/Offline',
        'pt' => 'On/Offline',
        'es' => 'On/Offline',
    ],
    'Schedule' => [
        'en' => 'Schedule',
        'pt' => 'Agenda',
        'es' => 'Agenda',
    ],
    'Schedules' => [
        'en' => 'Schedules',
        'pt' => 'Agendas',
        'es' => 'Agendas',
    ],
    'Scheduled' => [
        'en' => 'Scheduled',
        'pt' => 'Agendado',
        'es' => 'Agendado',
    ],
    'Booking' => [
        'en' => 'Booking',
        'pt' => 'Marcação',
        'es' => 'Booking',
    ],
    'Bookings' => [
        'en' => 'Bookings',
        'pt' => 'Marcações',
        'es' => 'Bookings',
    ],
    'Booked' => [
        'en' => 'Booked',
        'pt' => 'Marcado',
        'es' => 'Booked',
    ],
    'Cancelled' => [
        'en' => 'Cancelled',
        'pt' => 'Cancelado',
        'es' => 'Cancelado',
    ],
    'Suspended' => [
        'en' => 'Suspended',
        'pt' => 'Suspenso',
        'es' => 'Suspenso',
    ],
    'Postponed' => [
        'en' => 'Postponed',
        'pt' => 'Adiado',
        'es' => 'Adiado',
    ],




];
